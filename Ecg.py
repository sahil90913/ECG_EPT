...
class ECG:
	...
	def DimensionalReduciton(self,test_final):
		...
		pca_model_path = os.path.join(os.path.dirname(__file__), 'model', 'PCA_ECG.pkl')
	...
	def ModelLoad_predict(self,final_df):
		...
		"""
		model_path = os.path.join(os.path.dirname(__file__), 'model', 'voting_classifier.pkl')
		loaded_model = joblib.load(model_path)
		result = loaded_model.predict(final_df)
		if result[0] == 1:
			return "You ECG corresponds to Myocardial Infarction"
		elif result[0] == 0:
			return "You ECG corresponds to Abnormal"
		elif result[0] == 2:
